...
/**
 * Compte de versement mobile money du voyageur. Aucun endpoint n'accepte de numéro de
 * téléphone en entrée — {@link MobileMoneyAccountService#activate} le relit toujours chez
 * Firebase à partir de l'utilisateur authentifié. L'identifiant utilisateur vient uniquement
 * du contexte de sécurité (jamais d'un paramètre d'URL ou de corps), ce qui garantit qu'un
 * appelant n'agit que sur son propre compte.
 */
@RestController
...
public class MobileMoneyAccountController {
    ...
    /** Sans corps : le numéro vient du compte Firebase, jamais de la requête. */
    @PostMapping
    public MobileMoneyAccountResponse activate(@AuthenticationPrincipal String firebaseUid) {
        return service.activate(callerId(firebaseUid));
    }
