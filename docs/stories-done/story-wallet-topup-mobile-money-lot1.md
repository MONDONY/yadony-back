...
- `payments/wallet/WalletController.java` : trois points d'entrée ajoutés (`POST /wallet/topup/providers`, `GET /wallet/topup/{topupId}/status`) et branchement de `MOBILE_MONEY` sur `POST /wallet/topup` existant.
...
- **`WalletMobileMoneyTopupService.status()` et l'auto-invocation de `getOrCreate`** : `WalletService.getBalance`/`credit` appellent `getOrCreate` par auto-invocation interne à la classe (pas via le proxy Spring), donc la lecture du solde après confirmation n'ouvre pas une nouvelle transaction propre à `getOrCreate` par ce chemin ; `status` reste `@Transactional(readOnly = true)` et `WalletService.getOrCreate` est `NOT_SUPPORTED` (il suspend la transaction en cours pour pouvoir écrire malgré le mode lecture seule).
...
- `./mvnw test` : suite en cours d'exécution par le contrôleur au moment de la rédaction de cette documentation (voir marqueur ci-dessous pour le résultat final).
