...
**Règle produit** : le numéro fourni dans le corps de `POST /payments/mobile-money/account` est TOUJOURS prioritaire s'il est renseigné, même si le compte Firebase de l'appelant a déjà un téléphone — il peut légitimement différer (numéro d'un proche, opérateur distinct…). Il est normalisé par `Msisdn.normalize` puis validé par pawaPay comme n'importe quel numéro ; 422 `mobile-money-invalid-phone` s'il est inexploitable (erreur de saisie CLIENT, même motif que `BidService#createBid` — distincte du 502 d'un numéro Firebase ou prédit par pawaPay inexploitable, dont l'utilisateur n'est pas responsable). Sans numéro fourni (absent, vide ou blanc), le téléphone du compte Firebase (déjà vérifié par OTP) est utilisé s'il existe. Sans numéro d'aucune source : 422 `mobile-money-phone-required` (« Indiquez le numéro mobile money qui recevra vos versements. »). Réactiver avec un autre numéro fourni écrase le précédent : changement de numéro volontaire, pas une anomalie.

**Motif et risque accepté** : un numéro fourni peut légitimement différer du numéro Firebase, d'où sa priorité. Contrepartie assumée : contrairement au numéro Firebase, il n'est PAS vérifié par OTP — une session compromise (jeton volé, appareil déverrouillé) pourrait ainsi rediriger les versements futurs vers un numéro contrôlé par un tiers. Garde-fous actuels : la provenance est tracée dans l'audit `MM_ACCOUNT_ACTIVATED` (`source: "provided"` vs `"firebase"`), et `GET` ne renvoie jamais que le numéro masqué. À renforcer par une vérification OTP du numéro saisi dès que la vérification par SMS sera disponible.
