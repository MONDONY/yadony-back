...
- `db/migration/V246__pawapay_operations.sql`, `V247__users_mobile_money.sql`, `V248__payments_rail.sql`, `V249__bids_mobile_money.sql`.
...
- `payments/PaymentEntity.java` : `rail` (défaut `STRIPE`) ; `stripePaymentIntentId` devient nullable. Aucune colonne de référence vers `pawapay_operations` (les trois colonnes de confort de V248 ont été retirées par V250 : un seul lecteur, et toute la discipline « jamais de setter après un claim » n'existait que pour elles).
...
- **`PawapayOperationEntity`** (table `pawapay_operations`, `V246`) : **n'étend pas `BaseEntity`**, choix délibéré, l'`id` est assigné (c'est l'identifiant envoyé à pawaPay) et non généré, et une opération ne se supprime jamais, même logiquement. `@Version` est nullable pour que Spring Data traite l'entité comme neuve malgré l'id déjà posé. Colonnes : `kind` (`DEPOSIT`/`PAYOUT`/`REFUND`), `status` (8 valeurs), `amount`, `currency`, `provider`, `country`, `msisdn` (chiffré) + `msisdn_masked`, `payment_id` (FK `payments`, nullable), `related_operation_id` (pour un REFUND, le DEPOSIT d'origine), `authorization_url`, `provider_transaction_id`, `failure_code`/`failure_message`, `raw_callback` (chiffré, voir Pièges), quatre horodatages, `version`. Index `idx_pawapay_ops_open` (partiel, statuts non finaux, pour le poller) et **`uq_pawapay_ops_live_per_payment`** UNIQUE `(payment_id, kind)` partiel `WHERE status NOT IN ('FAILED','SUBMIT_REJECTED') AND payment_id IS NOT NULL` : au plus une opération vivante ou aboutie par paiement et par type, le verrou base contre le double deposit/payout/refund.
- **`users.mobile_money_*`** (`V247`) : `mobile_money_status` (NOT NULL, DEFAULT `NOT_CONFIGURED`), `mobile_money_msisdn` (chiffré), `mobile_money_msisdn_masked`, `mobile_money_provider`, `mobile_money_country`, `mobile_money_currency`, `mobile_money_verified_at`. Enum Java `MobileMoneyPayoutStatus` (`auth/`), méthode `UserEntity.hasActiveMobileMoney()`, source unique de la condition, jamais recopiée ailleurs.
- **`payments.rail`** (`V248`) : `VARCHAR(10) NOT NULL DEFAULT 'STRIPE'` + CHECK. `stripe_payment_intent_id` devient nullable (contrainte UNIQUE conservée, PostgreSQL et H2 acceptent plusieurs NULL). Les trois colonnes de confort `pawapay_deposit_id`/`pawapay_payout_id`/`pawapay_refund_id` que V248 avait aussi posées sont **retirées par `V250`** (`V250PaymentsDropPawapayRefsMigrationTest`) : le seul lien qui fait autorité est `pawapay_operations.payment_id`, le détail admin le lit via `PawapayOperationService.findLatest`.
- **`bids.mobile_money_phone`** (`V249`, élargie de `VARCHAR(30)` à `VARCHAR(255)` et désormais chiffrée) + `bids.mobile_money_country_code` (type inchangé, mais sa valeur est vidée par la même migration partout où `mobile_money_phone` l'est). La deadline de paiement mobile money **réutilise** la colonne existante `bids.awaiting_payment_expires_at` : pas de nouvelle colonne `payment_deadline_at` comme envisagé initialement dans la conception.
...
3. **L'autorité est `pawapay_operations.payment_id`.** Toute question « ce paiement a-t-il déjà un payout/refund/deposit ? » s'interroge sur `pawapay_operations` (`findLive`, `findLatest`), jamais sur `payments` — qui ne porte plus aucune référence d'opération depuis V250, précisément pour qu'aucune lecture ne puisse être en retard sur la réalité pawaPay.
...
- **Aucune référence d'opération pawaPay sur `payments`** (V250 retire les trois colonnes de confort de V248). Elles n'avaient qu'un lecteur (le détail admin) et imposaient, à chaque écriture après un claim en masse, des UPDATE ciblés dédiés (`attachPayoutId`/`attachRefundId`) pour ne jamais salir l'entité — un mécanisme qui a évité un bug sur chaque versement et chaque remboursement, mais que le prochain contributeur pouvait contourner par un simple setter. Sans colonne à poser, plus rien n'invite à écrire sur l'entité après un claim ; l'admin lit `pawapay_operations` (dernière opération de chaque type).
...
## Suite (2026-09-09) : numéro saisi à l'activation

**Règle produit** : le numéro du compte Firebase de l'appelant (vérifié par OTP) reste TOUJOURS la seule source dès qu'il existe — un numéro fourni dans le corps de `POST /payments/mobile-money/account` est alors ignoré en silence, sans erreur. Il n'est utilisé que si le compte Firebase n'a aucun téléphone, tant que la vérification par SMS (Twilio) n'est pas câblée pour ces comptes-là : sans cette tolérance, ils n'auraient aucun moyen d'activer le rail. Sans numéro d'aucune source : 422 `mobile-money-phone-required` (« Indiquez le numéro mobile money qui recevra vos versements. »). Un numéro fourni mais inexploitable par `Msisdn.normalize` : 422 `mobile-money-invalid-phone` (erreur de saisie CLIENT, même motif que `BidService#createBid` — distincte du 502 d'un numéro Firebase ou prédit par pawaPay inexploitable, dont l'utilisateur n'est pas responsable).

**Motif** : un numéro saisi de la sorte n'est pas vérifié par OTP — le risque d'un versement dirigé vers un mauvais numéro est documenté et assumé par le produit, faute d'alternative tant que la vérification SMS n'existe pas pour les comptes sans téléphone Firebase.

**Fichiers** :
- `payments/mobilemoney/dto/MobileMoneyActivateRequest.java` (nouveau) : corps optionnel `{phoneNumber}`, même patron que `MobileMoneyInitiateRequest`.
- `payments/mobilemoney/MobileMoneyAccountController.java` : `activate` accepte `@RequestBody(required = false) MobileMoneyActivateRequest body`, même patron que `MobileMoneyPaymentController#initiate`.
- `payments/mobilemoney/MobileMoneyAccountService.java` : nouvelle signature `activate(UUID userId, String providedPhone)` — Firebase prioritaire, sinon `Msisdn.normalize(providedPhone)` (422 `mobile-money-invalid-phone` si hors bornes), sinon 422 `mobile-money-phone-required`. La résolution pawaPay, le stockage et le masquage sont inchangés. Le payload d'audit `MM_ACCOUNT_ACTIVATED` gagne une clé `source` (`firebase` ou `provided`), jamais le numéro en clair.

**Tests** : `MobileMoneyAccountServiceTest` (+4 : numéro fourni utilisé quand Firebase n'en a pas avec résolution pawaPay appelée sur le numéro normalisé et audit `source=provided` ; Firebase prioritaire sur un numéro fourni avec audit `source=firebase` ; numéro fourni invalide sans Firebase → 422 `mobile-money-invalid-phone` sans écriture ; ni l'un ni l'autre → 422 `mobile-money-phone-required` avec le nouveau détail) ; `MobileMoneyAccountControllerIT` (+1 : corps avec numéro transmis brut, non normalisé, au service — POST sans corps toujours accepté, inchangé). Suite complète du package `com.yadony.api.payments.mobilemoney` (12 classes) : **137 tests, 0 échec, 0 erreur, BUILD SUCCESS** (2026-09-09).
