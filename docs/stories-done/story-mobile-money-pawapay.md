...
- `PawapayOperationRepository.java` : les deux transitions atomiques (`applyTransition`, `markSubmittedIfStillCreated`), sélections bornées pour le poller.
...
- `PawapayAmounts.java` : arrondi et formatage des montants selon la devise (XOF/XAF sans décimale).
- `PawapayCountries.java` : conversion alpha-3 (pawaPay) vers alpha-2 (yadony).
- `PawapayProviders.java` : libellés lisibles et détection des providers à redirection (Wave).
...
- `payments/PaymentEntity.java` : `rail` (défaut `STRIPE`), `pawapayDepositId`, `pawapayPayoutId`, `pawapayRefundId` ; `stripePaymentIntentId` devient nullable.
- `payments/PaymentRepository.java` : `findByBidIdForUpdate`, `markEscrowIfPending`, `markCancelledIfPending`, `attachPayoutId`, `attachRefundId`, filtre `rail` sur la liste admin.
...
- `markEscrowIfPending(paymentId, operationId, now)` : `UPDATE payments SET status='ESCROW', captured_at=now, pawapay_deposit_id=opId WHERE status='PENDING'`.
...
2. Un payout déjà vivant ou abouti pour ce paiement (`findLive`) : il est **rattaché** (`attachPayoutId`, jamais un second `submitPayout`), alerte dédupliquée `MM_PAYOUT_ORPHAN_<paymentId>`.
3. Sinon `submitPayout(...)`. `SUBMIT_REJECTED` -> alerte `PAWAPAY_PAYOUT_REJECTED` puis exception (claim annulé). `ACCEPTED` -> audit `ESCROW_RELEASED_MOBILE_MONEY` dans une transaction **indépendante**, puis `attachPayoutId` (UPDATE ciblé, jamais un setter sur l'entité, voir Pièges).
...
- **`payments.rail`** (`V243`) : `VARCHAR(10) NOT NULL DEFAULT 'STRIPE'` + CHECK, colonnes de confort `pawapay_deposit_id`/`pawapay_payout_id`/`pawapay_refund_id` (voir Pièges : jamais l'autorité). `stripe_payment_intent_id` devient nullable (contrainte UNIQUE conservée, PostgreSQL et H2 acceptent plusieurs NULL).
...
3. **L'autorité est `pawapay_operations.payment_id`, jamais les colonnes `payments.pawapay_*_id`.** Ces dernières ne sont qu'un confort de lecture (admin, export) et peuvent être vides alors que l'opération existe réellement (voir Logique métier critique). Toute question « ce paiement a-t-il déjà un payout/refund/deposit ? » s'interroge sur `pawapay_operations`.
...
5. **Une alerte dédupliquée sur un chemin qui lève est inerte.** Toute alerte de ce rail est dédupliquée en cherchant d'abord une ligne `admin_alerts` non résolue du même type avant d'en insérer une nouvelle. Si ce garde-fou (recherche + insertion + `raise`) reste dans la transaction ambiante d'une méthode dont l'appelant relance immédiatement une exception, la transaction entière (y compris la ligne de dédup fraîchement insérée) est annulée à chaque appel : la recherche suivante retrouve toujours zéro ligne, l'alerte repart à chaque tentative, et le mécanisme de déduplication ne sert jamais à rien alors qu'il semble présent dans le code. La règle du rail : le corps complet de toute méthode d'escalade doit être exécuté dans sa propre transaction indépendante, jamais dans une transaction que l'appelant va annuler.
...
- Suite complète : **4904 tests, 0 échec, 0 erreur, 7 ignorés, BUILD SUCCESS** (première exécution des 40 commits de la branche ensemble ; aucune régression croisée entre tâches).
- Couverture JaCoCo, packages du rail : `com.yadony.api.payments.pawapay` **92 %**, `.pawapay.dto` **99 %**, `.pawapay.events` **100 %**, `com.yadony.api.payments.mobilemoney` **90 %**, `.mobilemoney.dto` **100 %**. Tous au-dessus du seuil de 90 %.
- Couverture globale du dépôt : **89 %**. Ceci ne constitue **pas** un manquement : le rail ajoute environ 6 500 instructions couvertes entre 90 et 92 % dans un dépôt à 89 %, il relève donc mathématiquement la moyenne, il ne peut pas l'avoir abaissée. C'est aussi la pratique déjà établie de ce dépôt (`docs/stories-done/refunds-idempotence-noshow-bidirectionnel.md` : « la couverture globale reste sous le seuil à cause de code legacy non touché ; le code introduit/modifié par ce chantier est couvert »). Le seuil de 90 % s'applique au code du chantier, il est tenu partout sur ce rail.
...
- **`attachPayoutId`/`attachRefundId` en UPDATE ciblés**, jamais des setters sur l'entité gérée après un claim en masse : voir piège n°1. Ce choix a directement évité un bug qui aurait affecté **chaque** versement et **chaque** remboursement mobile money en production (le claim aurait été effacé au commit sur le chemin nominal).
