...
- **`AutomationRuleEntity` ne se persiste pas via JPA dans un test.** Ses champs `conditions` (`List<Map<String,Object>>`) et `action` (`Map<String,Object>`) sont mappés en `jsonb` via `@JdbcTypeCode(SqlTypes.JSON)` ; sous H2 la relecture échoue avec `Could not deserialize string to java type`. Aucun test du dépôt ne la persiste — tout le package `automation/` est testé unitairement avec un repository mocké. Suivre cette convention. **Ne jamais changer le mapping de ces colonnes pour faire passer un test** : elles stockent les règles de tous les voyageurs sur des colonnes `jsonb` PostgreSQL réelles.
