...
**Recette staging du 2026-09-09 (premiers vrais callbacks).** pawaPay a appelé `/api/v1/pawapay/callbacks/{deposits,refunds}` dès la soumission puis avec relances (+2, +6, +14, +24 min), toutes rejetées en 401 par le vérifieur sans aucune trace serveur : les opérations n'avançaient que par le poller, et l'absence de callback a d'abord été prise pour un problème de configuration du dashboard. Constats et suites :

- Le rejet est désormais journalisé en WARN par `PawapayCallbackController` : `pawaPay callback DEPOSIT : signature refusée (<motif>) ; authority=… signature-input=…`. Devant des 401 sur ces routes dans les journaux nginx, c'est la première chose à lire : `docker logs yadony_api | grep "signature refusée"`.
- pawaPay encode ses signatures ECDSA en **DER** (exemple `sig-pp=:MEQCI…:` de sa documentation « Signatures »), pas en `r || s` brut comme le prescrit RFC 9421 : le vérifieur accepte désormais les deux formes, discriminées par la longueur (64 octets P-256, 96 octets P-384 pour la forme brute).
- Point 4 ci-dessous : les relances tardives ont produit exactement la même réponse 401 que la première tentative, ce qui exclut un rejet « created trop ancien » : pawaPay re-signe ses relances.
- Points 1 et 2 : URL enregistrée avec `/api/v1` et `proxy_set_header Host $host` vérifiés sur staging.
