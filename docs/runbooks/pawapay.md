...
1. **Ne jamais se fier à `payments.pawapay_payout_id` pour trancher.** Cette colonne est un confort de lecture, elle peut être vide alors que l'opération existe réellement. La question « ce paiement a-t-il déjà un payout ? » se pose sur `pawapay_operations`, pas sur `payments`.
